<template>
  <v-container>
    <!-- Boîte de dialogue si l'organisation est introuvable -->
    <v-dialog v-model="showDialog" max-width="500px">
      <v-card>
        <v-card-title>Erreur</v-card-title>
        <v-card-text>
          Organisation introuvable ou clé secrète invalide.
        </v-card-text>
        <v-card-actions>
...
export default {
  ...
  computed: {
    ...mapState("org", ["currentOrg", "secretError"]),
  ...
  methods: {
    ...
    async fetchOrgDetails() {
      try {
        ...
        if (this.secretError !== 0 || !this.currentOrg) {
